...
YAML_CLEAN = """\
# Pronunciation Assessment — Colab Edition (GPU only, no HuggingFace auto-download)
project:
  seed: 42
  device: cuda
...
asr:
  model_name: /content/drive/MyDrive/SpeakAI-Eval/pretrained_models/whisper-large-v3-turbo
  language: en
  device: cuda
  torch_dtype: float16
  max_new_tokens: 440
...
wavlm:
  model_name: /content/drive/MyDrive/SpeakAI-Eval/pretrained_models/wavlm-large
  freeze: true
  use_lora: false
...
def writefile_cells(rel_path, content, base_dir="/content/drive/MyDrive/SpeakAI-Eval"):
    """
...
setup_cells.append(md_cell([
    "# SpeakAI-Eval — Setup Environment on Google Drive\n",
    "\n",
    "Notebook này chỉ cần chạy **MỘT LẦN DUY NHẤT** để:\n",
    "1. Ghi toàn bộ mã nguồn vào Google Drive (`MyDrive/SpeakAI-Eval`).\n",
    "2. Tải model ECAPA và lưu vào Drive.\n",
    "3. Khởi tạo thư mục cache cho Qwen LLM.\n",
    "\n",
    "**Yêu cầu:** Cho phép mount Google Drive khi chạy."
]))
...
setup_cells.append(code_cell([
    ...
    "print('Directories created on Drive.')"
]))

setup_cells.append(md_cell(["---\n", "### Ghi Source Code vào Drive"]))
for rel_path in INCLUDE_FILES:
    content = OVERRIDES.get(rel_path) or read_file(rel_path)
    if content is None:
        print(f"WARN: khong tim thay {rel_path}")
        continue
    for cell in writefile_cells(rel_path, content, base_dir="/content/drive/MyDrive/SpeakAI-Eval"):
...
setup_cells.append(code_cell([
    ...
    "    print('Downloading ECAPA model to Drive...')\n",
    ...
    "\n",
    "hp_path = f'{ECAPA_MODEL_DIR}/hyperparams.yaml'\n",
    "with open(hp_path, 'r', encoding='utf-8') as f:\n",
    "    content = f.read()\n",
    "content = re.sub(r'pretrained_path:\\s*.*', f'pretrained_path: {ECAPA_MODEL_DIR}', content)\n",
    "with open(hp_path, 'w', encoding='utf-8') as f:\n",
    "    f.write(content)\n",
    "print(f'Patched hyperparams.yaml')\n",
    "\n",
    "DRIVE_MODELS = '/content/drive/MyDrive/SpeakAI_Models'\n",
    "if os.path.exists(f'{DRIVE_MODELS}/transformer_models/pronunciation.pt'):\n",
    "    os.system(f'cp {DRIVE_MODELS}/transformer_models/pronunciation.pt {TARGET_DIR}/transformer_models/')\n",
    "    print('Copied pronunciation.pt')\n",
    "if os.path.exists(f'{DRIVE_MODELS}/pretrained_models'):\n",
    "    print('Copied pretrained_models')\n",
    "print('ALL SETUP COMPLETED!')"
]))

setup_cells.append(md_cell(["---\n", "### Tải LLM Qwen, Whisper, WavLM và Silero VAD về Drive"]))
setup_cells.append(code_cell([
    "import os\n",
    "import torch\n",
...
')\n",
    ...
    "    try:\n",
    "        # Lưu audio hội thoại tạm thời\n",
    "        conv_path = f'/tmp/SpeakAI_Audio/temp_api_{audio.filename}'\n",
...
save_notebook(setup_cells, "SpeakAI-Colab-Setup.ipynb")
save_notebook(run_cells, "SpeakAI-Colab-Run.ipynb")
