...
run_cells.append(code_cell([
      ...
      "    bad_words_str = ', '.join([f\"{w['word']} ({w.get('score',0):.1f})\" for w in bad_words])\n",
      "    bad_ph_str = ', '.join([f\"{p['phoneme']} ({p.get('score',0):.1f})\" for p in bad_ph])\n",
      "    \n",
      "    if bad_words_str or bad_ph_str:\n",
      "        pronunciation_feedback = f\"- Phát âm:\\n❌ Từ phát âm yếu: {bad_words_str}\\n❌ Âm vị cần sửa: {bad_ph_str}\\n\"\n",
      "    else:\n",
      "        pronunciation_feedback = \"- Phát âm: ✅ Tốt\\n\"\n",
      "    \n",
      "    prompt = f\"\"\"Bạn là giáo viên tiếng Anh.\\n",
      "Hãy đánh giá NGỮ PHÁP và NGỮ CẢNH của câu trả lời sau:\\n",
      "Ngữ cảnh (Giáo viên nói): {teacher_text}\\n",
      "Học viên trả lời: {student_text}\\n",
      "\\n",
      "Nếu câu trả lời lạc đề hoặc sai ngữ cảnh, hãy gợi ý một câu trả lời TỰ NHIÊN và PHÙ HỢP với câu hỏi của Giáo viên.\\n",
      "Hãy trả lời NGẮN GỌN THEO ĐÚNG ĐỊNH DẠNG SAU, KHÔNG THÊM BẤT KỲ LỜI KHUYÊN HAY CÂU CHÀO HỎI NÀO KHÁC:\\n",
      "- Ngữ pháp & Ngữ cảnh: ✅ Tốt. (nếu đúng) HOẶC ❌ [Lỗi gì] -> Gợi ý sửa: [câu sửa lại].\\n",
      "\"\"\"\n",
      "    messages = [{'role': 'system', 'content': 'Bạn là giáo viên tiếng Anh. Chỉ trả lời đúng định dạng yêu cầu, không thêm bất kỳ văn bản nào khác.'}, {'role': 'user', 'content': prompt}]\n",
      "    text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)\n",
      "    model_inputs = tokenizer([text], return_tensors='pt').to(llm_model.device)\n",
      "    generated_ids = llm_model.generate(**model_inputs, max_new_tokens=150)\n",
      "    generated_ids = [output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated_ids)]\n",
      "    llm_feedback = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0].strip()\n",
      "    return pronunciation_feedback + llm_feedback\n",
      "\n",
