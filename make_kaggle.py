...
run_cells.append(code_cell([
    ...
    "        pronun_info = f\"Lỗi phát âm hệ thống nhận diện: từ [{bad_words_str}], âm vị [{bad_ph_str}].\"\n",
    "    else:\n",
    "        pronun_info = \"Phát âm tốt, không phát hiện lỗi nghiêm trọng.\"\n",
    "    \n",
    "    prompt = f\"\"\"Bạn là giáo viên tiếng Anh. Hãy nhận xét câu trả lời của học viên.\\n",
    "Ngữ cảnh (Giáo viên hỏi/nói): {teacher_text}\\n",
    "Học viên trả lời: {student_text}\\n",
    "{pronun_info}\\n",
    "\\n",
    "Hãy đánh giá tổng hợp về ngữ pháp, ngữ cảnh và phát âm. Trả lời NGẮN GỌN, TRỰC TIẾP THEO ĐÚNG ĐỊNH DẠNG SAU (không thêm bất kỳ lời chào hỏi hay giải thích nào khác):\\n",
    "👍 Điểm tốt: [1 câu khen ngợi ngắn gọn]\\n",
    "⚠️ Cần cải thiện: [Lỗi quan trọng nhất về ngữ pháp, ngữ cảnh hoặc phát âm]\\n",
    "💡 Gợi ý: [Cách sửa lỗi phát âm, hoặc gợi ý câu trả lời tự nhiên hơn]\"\"\"\n",
    "    messages = [{'role': 'system', 'content': 'Bạn là giáo viên tiếng Anh tận tâm. Chỉ trả lời đúng định dạng yêu cầu, không thêm bất kỳ văn bản nào khác.'}, {'role': 'user', 'content': prompt}]\n",
    "    text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)\n",
    "    model_inputs = tokenizer([text], return_tensors='pt').to(llm_model.device)\n",
    "    generated_ids = llm_model.generate(**model_inputs, max_new_tokens=256)\n",
    ...
    "\\n",
    "Hãy viết tổng kết đánh giá chi tiết THEO ĐÚNG ĐỊNH DẠNG SAU (không thêm lời chào hay giải thích ngoài luồng):\\n",
    "🌟 Điểm mạnh:\\n",
    "[Liệt kê điểm mạnh chung của học viên]\\n",
    "🎯 Cần cải thiện:\\n",
    "[1-2 điểm cần cải thiện nhất trong toàn bộ đoạn hội thoại]\\n",
    "💪 Gợi ý luyện tập:\\n",
    "[Đề xuất phương pháp luyện tập cụ thể mỗi ngày]\"\"\"\n",
    "    messages = [\n",
    "        {'role': 'system', 'content': 'Bạn là giáo viên tiếng Anh tận tâm. Hãy trả lời đúng định dạng yêu cầu.'},\n",
