...
run_cells.append(code_cell([
    ...
    "    \"\"\"Fallback turn-level feedback when transformer_feedback is not available.\"\"\"\n",
    "    bad_words = [w for w in errors.get('words', []) if w.get('score', 10) < 7.0]\n",
    "    bad_ph = [p for p in errors.get('phonemes', []) if p.get('score', 10) < 7.0]\n",
    "    parts = []\n",
    "    if bad_words:\n",
    "        word_strs = [f\"{w['word']} ({w['score']:.1f})\" for w in bad_words[:5]]\n",
    "        parts.append(f\"Từ phát âm yếu: {', '.join(word_strs)}\")\n",
    "    if bad_ph:\n",
    "        ph_strs = []\n",
    "        for p in bad_ph[:5]:\n",
    "            tip = p.get('tip', '')\n",
    "            s = f\"/{p['phoneme']}/ ({p['score']:.1f})\"\n",
    "            if tip: s += f\" — {tip}\"\n",
    "            ph_strs.append(s)\n",
    "        parts.append(f\"Âm cần luyện: {'; '.join(ph_strs)}\")\n",
    "    if parts:\n",
    "        return '⚠️ Cần cải thiện: ' + '. '.join(parts)\n",
    "    if score >= 8.0:\n",
    "        return '✅ Phát âm tốt!'\n",
    "    elif score >= 6.0:\n",
    "        return '👍 Phát âm khá, tiếp tục luyện tập.'\n",
    "    return ''\n",
    "\n",
    "def generate_overall_summary(result):\n",
    "    \"\"\"Generate overall assessment summary using transformer feedback or scores.\"\"\"\n",
    "    # Try to use overall_transformer_feedback first\n",
    ...
    "    \n",
    "    has_l2_mdd = result.get('has_l2_mdd', False)\n",
    "    model_info = ' (Ensemble: SpeechOcean762 + L2-MDD)' if has_l2_mdd else ' (SpeechOcean762)'\n",
    "    \n",
    "    summary = f\"\"\"## {level}{model_info}\n",
