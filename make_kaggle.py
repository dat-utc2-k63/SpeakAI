...
setup_cells.append(code_cell([
    ...
    "subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', 'huggingface_hub', 'modelscope'], check=True)\n",
    "from modelscope.hub.snapshot_download import snapshot_download\n",
    "import os, re\n",
    "\n",
    "TARGET_DIR = '/kaggle/working/SpeakAI-Eval'\n",
    "ERES_MODEL_DIR = f'{TARGET_DIR}/pretrained_models/speech_eres2net_large_200k_sv_zh-cn_16k-common'\n",
    "os.makedirs(ERES_MODEL_DIR, exist_ok=True)\n",
    "\n",
    "if not os.path.exists(f'{ERES_MODEL_DIR}/model.onnx') and not os.path.exists(f'{ERES_MODEL_DIR}/pytorch_model.bin'):\n",
    "    print('Downloading ERes2Net-Large to Kaggle...')\n",
    "    snapshot_download(\n",
    "        'damo/speech_eres2net_large_200k_sv_zh-cn_16k-common',\n",
    "        local_dir=ERES_MODEL_DIR\n",
    "    )\n",
    "    print('Download done!')\n",
    "else:\n",
...
run_cells.append(code_cell([
    ...
    "    'python-dotenv>=1.0.0', 'deepfilternet', 'modelscope',\n",
...
run_cells.append(code_cell([
    ...
    "    \n",
    "    if cf_token:\n",
    "        print('Found CF_TOKEN in Kaggle Secrets! Using Named Tunnel...')\n",
    "        cmd = f'cloudflared tunnel run --token {cf_token}'\n",
    "    else:\n",
    "        print('CF_TOKEN not found! Falling back to Quick Tunnel...')\n",
    "        cmd = f'cloudflared tunnel --url http://127.0.0.1:{port}'\n",
    "        \n",
    "    process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)\n",
    "    url = None\n",
    "    if cf_token:\n",
    "        # Named tunnel won't print trycloudflare.com url, you already know your domain\n",
    "        url = 'YOUR_NAMED_TUNNEL_DOMAIN' # Trình quản lý DNS Cloudflare của bạn\n",
    "        return url\n",
    "        \n",
    "    for _ in range(20):\n",
...
')\n",
    ...
    "        audio_path = f'/tmp/SpeakAI_Audio/{temp_id}_{audio.filename}'\n",
    "        with open(audio_path, 'wb') as f:\n",
    "            shutil.copyfileobj(audio.file, f)\n",
    "        \n",
    "        # Tải audio và resample về 16kHz\n",
    "        from infer.device_utils import load_audio\n",
    "        waveform, sr = load_audio(audio_path, target_sr=16000)\n",
    "        \n",
    "        # Khử nhiễu nếu có DeepFilterNet trong pipeline\n",
    "        if hasattr(pipeline.diarization, 'denoiser') and pipeline.diarization.denoiser:\n",
    "            waveform = pipeline.diarization.denoiser.denoise(waveform, sr)\n",
    "            \n",
